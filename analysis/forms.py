...
class FillQueryForm(forms.Form):
	...
	where = forms.CharField(label="WHERE:", required=False, widget=forms.TextInput(attrs={'placeholder': 'Conditions on column values, separated by AND. Leave blank if none.'}))
	limit = forms.CharField(label="LIMIT:", required=False, widget=forms.TextInput(attrs={'placeholder': 'Numerical value limiting number of rows returned.'}))
