...
class FillQueryForm(forms.Form):
	...
	where = forms.CharField(label="WHERE:", widget=forms.TextInput(required=False, attrs={'placeholder': 'Conditions on column values, separated by AND. Leave blank if none.'}))
	limit = forms.CharField(label="LIMIT:", widget=forms.TextInput(required=False, attrs={'placeholder': 'Numerical value limiting number of rows returned.'}))
